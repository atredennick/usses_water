...
We first averaged the observed soil moisture for each day (*d*) and plot (*i*), $x_{i,d}$.
\new{Then we standardized the average of each plot within a plot group ($g$) by subtracting the mean ambient soil moisture of the plot group for the day ($\overline{x}_{g,d} = \sum^{i=1}_G x_{i,d} / n$) from each plot's daily average and dividing by the standard deviation of the ambient soil moisture in that plot group ($\sigma_{g,d}$): $x^*_{i,d} = \frac{x_{i,d} - \overline{x}_{g,d}}{\sigma_{g,d}}$.}
