...
\new{We used the relocated plots as our controls because demographic data is difficult to collect and is already being collected in these plots for other ongoing studies.}
...
\new{This metric of sensitivity measures the response of ANPP standardized to the magnitude of the treatment. 
...
\new{We conducted the above analysis for all species, and then conducted a separate analyses for annual species only (Appendix 1). 
Annual species have shorter life spans, so conducting a separate analysis allows us to discover if community responses are restricted to short-lived species that might respond more quickly to altered precipitation regimes. 
Given the dominance of perennial species in our system (Appendix 1), shifts in the annual plant community could be masked in the analysis of the full community.}
...
\new{Sensitivity of ANPP to irrigation was constant over the course of our experiment (Fig. 3D). Sensivitity of ANPP to drought, however, grew over time (\emph{P} = 0.0002; Fig. 3D)}.
...
\new{We did find that ANPP became more sensitive to drought over time (Fig. 3D).
Thus, the difference between ANPP, standardized by the difference in soil moisture, in drought and control plots grew over time.
