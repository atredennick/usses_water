...
\renewcommand\thefigure{Ax-\arabic{figure}}
\renewcommand\theequation{Ax.\arabic{equation}}
\renewcommand\thetable{Ax-\arabic{table}}  
\renewcommand\thesection{Section Ax\arabic{section}}


\begin{center}
\textbf{\Large{Appendix X}} \\
...
# Details on the plant community and dynamics
...
Four to five species dominate the community in general (Figure Ax-1), indicating a high level of dominance in this plant community.
...
Annual plant community composition is relatively stable through time (Fig. Ax-5) and in most years there is no evidence that treatment differentiates community composition (Table Ax-).
...
Table Ax-1: Statistical results from linear model relating density of Allysum desertorum to precipitation.
```{r report_models, echo = FALSE}
summary(lm(mean_density~ppt1*Treatment, data = filter(dom_annuals, species == "Allyssum desertorum")))
```

\newpage{}
Table Ax-2: Statistical results from linear model relating density of Bromus tectorum to precipitation.
```{r report_models2, echo = FALSE}
summary(lm(mean_density~ppt1*Treatment, data = filter(dom_annuals, species == "Bromus tectorum")))
```
...
ggplot(cover_data, aes(x = mean_rank, y = mean_area))+
  ...
  xlab("Species rank")+
  ylab(expression(paste("Mean basal area (",m^2,")")))+
...
```{r time_perr_racs, echo=FALSE, include=TRUE, message=FALSE, warning=FALSE, fig.height=6, fig.cap="Time series of rank abundance curves for perennial species in each treatment. Values of mean area were averaged over plots. The four most dominant species are labelled in each panel."}
cover_data <- {}
for(dotrt in unique(cover_community_matrix$Treatment)){
...
ggplot(dom_annuals, aes(x = ppt1, y = mean_density, color = Species))+
  geom_point()+
