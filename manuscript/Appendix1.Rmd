...
We took four readings per plot that were averaged.
...
| Band number | Spectrum name | Spectrum wavelengths | Corresponding satellite |
| ----------- | ------------- | -------------------- | ----------------------- |
| 1 | red | xx-xx $\mu$m | AVHRR |
| 2 | red | xx-xx $\mu$m | MODIS |
| 3 | near-infrared | xx-xx $\mu$m | AVHRR |
| 4 | near-infrared | xx-xx $\mu$m | MODIS |
...
\begin{align}
\text{NDVI}_{\text{AVHRR}} &= \frac{b_3\delta_{1(\text{AVHRR})} - b_1\delta_{2(\text{AVHRR})}}{b_3\delta_{1(\text{AVHRR})} + b_1\delta_{2(\text{AVHRR})}} \\[10pt]
\text{NDVI}_{\text{MODIS}} &= \frac{b_4\delta_{1(\text{MODIS})} - b_2\delta_{2(\text{MODIS})}}{b_4\delta_{1(\text{MODIS})} + b_2\delta_{2(\text{MODIS})}}
\end{align}

\noindent{} where $b_x$ refers to band *x* (*x* = 1,2,3,4) in Table A1-1 and $\delta$s are scaling factors unique to each band.
The values for the scaling factors come from \textcolor{red}{(PETER?)} and are as follows: $\delta_{1(AVHRR)} = 0.77$, $\delta_{2(AVHRR)} = 1$, $\delta_{1(MODIS)} = 0.95$, $\delta_{2(MODIS)} = 0.96$.
...
tab_dat <- read.csv("../data/estimated_biomass/ndvi_biomass_regression_parameters.csv", row.names = 1) %>%
  filter(year > 2011)
colnames(tab_dat) <- c("Year", "Intercept", "NDVI Slope", "$R^2$", "Min(biomass)", "Max(biomass)", "Algorithm")
