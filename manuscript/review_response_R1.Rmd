...
\response{This is an interesting point, and one we failed to discuss. We have added some text to our discussion on this topic (lines xxx-xxx).}
...
\response{These are all good suggestions. Reviwer 2 agreed that the paper would benefit from a broader introduction, which we have added (lines xx-xx). We have also added a brief introduction to the idea that domoninant species play a large role in determining ecosystem functioning (lines xx-xx).}
...
\response{We have revised this sentence to read, "xxx" (lines xx-xx).}
...
\response{Fair point, and one that echoes Reviewer 2's critique of our introduction. We have added a new opening paragraph that we hope better sets the stage.}
...
\response{Upon re-reading, we completely agree! We have substantially rewritten this section for clarity (lines xxx-xxx). Hopefully it is less confusing now.}
...
\response{Sorry about this mistake. We actually only use \emph{cumulative} volumetric water content and have corrected this in the manuscript.}
...
\response{We have increased the contrast between colors so that they are easier to distinguish if printed in black and white.}