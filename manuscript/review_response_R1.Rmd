...
\response{We provide point-by-point responses to reviewer comments below. We have taken care to address Reviewer 1's concerns (also echoed by Reviewer 2) by substantially revising our Introduction section. In particular, we have shifted our focus away from the Hierarchical Response Framework, and instead focus on how species' responses and community compositional shifts can impact ecosystem functional response. We do this, rather than implementing the suggestions of Reviewer 1, because our data limits the complexity of our statistical model. Within a year and treatment, each plot has the same value of volumetric water content. Therefore, we simply don't have the variation needed to fit a VWC$\times$Treatment$\times$Year interaction, which would be necessary to incorporate the effect of time on the ecosystem functional response.} 

\response{We therefore use the Hierarchical Response Framework to support our hypotheses about potential mechanisms driving change in ecosystem functional response (species' responses and community change), but refrain from suggesting we are explicitly testing the Hierarchical Response Framework. We also circle back to this point in our Discussion (lines xxx-xxx). Fortunately, our community-level analysis does have an explicit temporal component, which does link back to the Hierarchical Response Framework.}
...
\response{This is a great suggestion, and one we had not thought of. We conducted the analysis suggested by the reviewer and found that community composition was stable for both annuals (short-lived) and perennials (long-lived). In a few years we detected signficant treatment effects on dispersion or mean of community composition. But upon further investigation, we found that these statistical results were detecting changes in rare species because the dominant annuals showed similar dynamics through time, regardless of treatment (Fig. Ax-x). Likewise, the stable perennial community contributes most to ANPP is this systems. We briefly mention this analysis in our Results section on lines xxx-xxx, and show the full analysis in Appendix X.}
...
\response{We have added a new appendix (Appendix X) that provides more details on the dominance and rarity of the communities across and through time. In brief, rank abundance curves are very similar for all treatments and through time, with the same two perennial species (\emph{Artemisia tripartita} and \emph{Balsamorhiza sagittata}) dominant in all treatments, acounting for about 25\% of total basal cover. Similarly, all treatments had the same dominant annual species, whose dynamics were more associated with interannual variation in precipitation than to variation among treatments. These ancillary results add weight to our conclusion that the ecosystem responses is driven by dominant species, which are similar among treatments.}
