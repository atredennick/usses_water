...
m1 <- lmer( y ~ Treatment*vwc_scaled*year_id + (vwc_scaled|quadname), my_df)
...
rt  <- stanc("alt_anpp_randcoefs.stan")
sm  <- stan_model(stanc_ret = rt, verbose = FALSE)
fit <- sampling(sm, data = anppdat, iter = 2000, chains = 4, thin = 10)
