...
fit_stan_model <- function(model_data, check_diags=FALSE, treattype){
  lmod <- lm(log(anpp) ~ Treatment, model_data)
  x <- model.matrix(lmod)
  anppdat <- list(Nobs = nrow(model_data),
                  Npreds = ncol(x),
                  Nplots = length(unique(model_data$quadname)),
                  Nyears = length(unique(model_data$year_id)),
...
saveRDS(all_fit, "../results/randyears_alltreatments_fit.RDS")
